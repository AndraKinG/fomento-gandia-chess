...
### Adenda 1C (revisión final, item 8)

Dos excepciones/endurecimientos sobre la matriz de arriba, introducidos por el validador de convocatorias (Fase 1C) y su revisión final:

1. **Excepción SANCIONADA a la privacidad de borradores**: la RLS de `lineups` (migración 0005) solo deja ver un BORRADOR a su propio capitán o a un admin. Sin embargo, `cargarContextoValidacion` (src/lib/convocatorias/contexto-bd.ts) usa el cliente admin para leer, también, los BORRADORES de OTROS equipos del club que juegan la misma fecha o la misma sede — necesario para las comprobaciones cruzadas en vivo R7 (arts. 54-55, "dos actas el mismo día") y R8 (art. 52.4, "misma sede como un solo equipo"): un capitán no puede recibir un aviso sobre un solape con la convocatoria de otro equipo si esa convocatoria le es invisible. Esta lectura cruzada es una excepción DELIBERADA y acotada (solo dentro de `validarContexto`, nunca se expone el contenido del borrador ajeno tal cual — solo se usa para derivar avisos/errores sobre la propia alineación); el propio borrador ajeno nunca dejó de ser invisible por RLS directa. Ver también el comentario en `tipos.ts` (`ContextoClub.alineacionesMismaFecha`/`mismaSede`, campo `estado`): un solape contra un borrador ajeno es solo "aviso" (no bloquea), contra una convocatoria ya publicada sigue siendo "error".
2. **Transiciones de estado de `lineups` endurecidas en BD**: la migración `0007_blindaje_convocatorias.sql` (gate usuario, no aplicada automáticamente) añade un trigger `BEFORE UPDATE OR DELETE` en `public.lineups` que (a) congela cualquier UPDATE/DELETE si el encuentro ya está `jugado`, y (b) rechaza la transición `borrador -> publicada` salvo que la conexión sea `service_role`. Esto convierte al validador de servidor (`publicarConvocatoria`) en la ÚNICA puerta de publicación posible: ni siquiera un PATCH directo a PostgREST con el JWT del propio capitán (que la RLS normal SÍ permitiría, porque solo comprueba "es capitán de este match") puede saltarse la validación reglamentaria. `publicarConvocatoria`/`despublicarConvocatoria` (src/app/equipos/[id]/convocatoria/actions.ts) escriben la transición final con `createAdminClient()` precisamente para atravesar este trigger tras validar; `guardarBorrador` sigue con el cliente de usuario normal (RLS ya cubre bien la edición de un borrador propio, y el trigger no interviene en transiciones borrador→borrador).
