...
/**
 * Enciende el motor en una partida de RETO (migración 0044).
 *
 * PARA QUÉ: el propietario quiere vacilar un rato a un colega del club y contárselo
 * después. No es un modo de juego ni una función del club: es una broma suya, y por eso
 * está atada por todos lados.
 *
 * SOLO UNA FICHA, la que diga `FICHA_CON_MOTOR` en el entorno del servidor. No vale
 * "ser admin": lo pidió así expresamente —"aunque haya más admins, solo yo"— y además
 * un permiso ligado al rango se heredaría solo el día que nombre a alguien. Sin esa
 * variable puesta, esto no lo puede hacer nadie.
 *
 * SOLO EN RETOS. En un torneo interno hay clasificación y resultados que cuentan, así
 * que ahí no es una broma. Se comprueba aquí y lo impide además un CHECK de la base.
 *
 * SOLO SI JUEGAS TÚ. Un espectador con el motor no está vacilando a nadie.
 *
 * SE ENCIENDE Y SE QUEDA: no hay forma de borrar la marca. Apagarlo es cosa del
 * navegador —deja de pedirle jugadas— pero la partida seguirá diciendo que se jugó con
 * motor, y lo dirá al acabar, a los dos. Sin eso, esto sería otra cosa.
 */
export async function activarMotor(partidaId: string): Promise<Respuesta> {
  const ficha = process.env.FICHA_CON_MOTOR;
  const sesion = await sesionActual();
  if (!ficha || !sesion?.playerId || sesion.playerId !== ficha) {
  ...
  const { data: fila } = await db
    .from("live_games")
    .select("id, blancas_id, negras_id, origen, resultado, motor_ficha")
    .eq("id", partidaId)
    .maybeSingle();
  if (!fila) return { error: "Esa partida no existe." };
  if (fila.origen !== "reto") return { error: "En los torneos del club, no." };
  if (fila.resultado !== null) return { error: "Esa partida ya ha terminado." };
  if (fila.blancas_id !== ficha && fila.negras_id !== ficha) {
    return { error: "Solo en tus partidas." };
  }
  if (fila.motor_ficha) return {};

  const { error } = await db
    .from("live_games")
    .update({ motor_ficha: ficha })
    .eq("id", partidaId);
